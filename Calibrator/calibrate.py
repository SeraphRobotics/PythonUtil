...
def addLayer(tree, p_width, height, start_x, start_y, start_z, s):
    ...
    materialIdEl = etree.SubElement(path, "materialid").text = "1"
    #path.append(materialIdEl)
    point_list = []
    coordinate_list = []
...
def createTransition(start, end, clear, spd):
    ...
    for item in [speed, start, clear_a, clear_b, end]:
...
def generateSample(tip, out_f, s, p_width, p_speed, p_height, height, q, a_const,c_vol):
    ...
    height_count = 20#s - p_height  #cludge to set height
    layer_count = 0
    ...
    while height_count > 0.0:
        ...
        commands.append(createTransition(start, end, 2 * p_height, p_speed))
        commands.append(pointsToPath(material, new_layer))
 #       for pathitem in createTransition(start, end, 2 * p_height, p_speed):
 #           commands.append(pathitem)
        
#        for pathitem in pointsToPath(material, new_layer):
#            commands.append(pathitem)
