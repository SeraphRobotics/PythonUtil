...
def selectSample(filename):
	...
	random.seed()
	while ((len(sample) < 500) and (len(doctors)>1)):
		s = random.randint(0, len(doctors) - 1)
		print s
		print len(doctors), doctors        
		if doctors[s][2] not in zips:
...
if len(sys.argv) < 3:
	print "Usage: python faxer.py <database> <path_to_template>" 
	sys.exit(0)
	
f = open("METADATA.txt", "a+")
faxes, sample = selectSample(sys.argv[1])
for s in sample:
